...
|---|---|---|
| `ortsbezirk_id` | Text | Offizielle ID des Ortsbezirks, **Primaerschluessel**. Fuehrende Null erhalten (z. B. `01`). Nicht fortlaufend von 1 bis 26. |
| `name_offiziell` | Text | Offizielle Schreibweise des Namens (z. B. `Westend/Bleichstraße`, `Mainz-Kastel`). |
...
| `ortsbezirk_id` | Text | Die ID, auf die der Alias abgebildet wird. Verweis auf `ortsbezirke_referenz.csv`. |
...
Eigenschaften je Feature: `ortsbezirk_id`, `name_offiziell`, `osm_id`, `akk`.
Fuer Flaechen- oder Distanzberechnungen in Metern nach **EPSG:25832** transformieren.

## Nutzungsregel (Datenstandard)

1. Jeder ortsbezirksbezogene Datensatz fuehrt `ortsbezirk_id` als **Pflichtfeld**.
...
- **Quellen:** offizielle Ortsbezirks-IDs und -Namen der Stadt; Geometrie aus
  OpenStreetMap (ODbL), Grenz-Relationen `admin_level=9`.
